...
void FontManager::renderText(Surface* surface, const std::string& text, float x,
                             float y, Color color, const Rect& clipRect)
{
  if (!face) {
  ...
  // 获取surface的bitmap
  Bitmap* bitmap = surface->lockBuffer();
  if (!bitmap) {
  ...
  while (*p) {
    ...
    if (FT_Load_Char(face, charCode, loadFlags)) {
      LOGE("FontManager::renderText - Failed to load glyph for character code: "
           "%u",
           charCode);
      continue;
    }
    ...
    // 渲染字形
    renderGlyph(bitmap, ftBitmap, x0 >> 6, y0 >> 6, color, clipRect);
...
void FontManager::renderGlyph(Bitmap* bitmap, const FT_Bitmap& ftBitmap, int x,
                              int y, Color color, const Rect& clipRect)
{
  // 遍历字形的每个像素
  for (unsigned int row = 0; row < ftBitmap.rows; row++) {
    ...
    for (unsigned int col = 0; col < ftBitmap.width; col++) {
      ...
      // 获取字形像素的alpha值
      unsigned char alpha = ftBitmap.buffer[row * ftBitmap.pitch + col];
      if (alpha == 0)
        continue; // 跳过完全透明的像素

      // 读取背景色
      Color bg = bitmap->getPixel(dx, dy);

      // Alpha混合
      float a = alpha / 255.0f;
      Color blended(static_cast<uint8_t>(color.r * a + bg.r * (1 - a)),
                    static_cast<uint8_t>(color.g * a + bg.g * (1 - a)),
                    static_cast<uint8_t>(color.b * a + bg.b * (1 - a)),
                    255 // 保持完全不透明
      );

      // 写回混合后的颜色
      bitmap->setPixel(dx, dy, blended);
    }
  }
}
